...
The LDES Server is one of the components built as part of a large project on creating a [data space for Flanders](https://www.vlaanderen.be/vlaamse-smart-data-space-portaal) (sorry, Dutch only) for Data Owners, Data Publishers and Data Clients. Its main purpose is to accept data set members, store them in external storage and allow to retrieve the data set as a LDES for replication as a whole or just a subset of the data set. However, the real benefit of LDES lies in its ability to allow Data Clients to keep in sync with changes that occur on the data set.
